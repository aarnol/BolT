...
saved = pd.DataFrame(columns=["Atlas", "Invert", "Accuracy","Precision", "Recall", "ROC"])
for invert in [True, False]:
        print(f"Running {invert}")
        ...
        print(preds)
        print(labels)
        saved.loc[len(saved)] = ["sphere30",  invert, accuracy, precision, recall, roc]

saved.to_csv(f"{argv.name}.csv", index=False)
